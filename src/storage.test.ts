...
test('保存した設定を読み戻せる', async () => {
  ...
  await saveSettings(b, { language: 'ja', region: 'america', finisher: 'quote', testMode: false })
  const s = await loadSettings(b)
  expect(s).toEqual({ language: 'ja', region: 'america', finisher: 'quote', testMode: false })
})
